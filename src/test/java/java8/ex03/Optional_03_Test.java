...
public class Optional_03_Test {
	...
	public void test_getAccountNull() throws Exception {
		...
		// TODO A l'aide de la méthode map récupérer le prénom (account -> person ->
		// firstname)
		...
		assertThat(accOpt.orElseThrow(GoodException::new), hasProperty("firstname", is("A")));
	}
	...
	public void test_getAccountWithPersonNull() throws Exception {
		...
		// TODO Utiliser la méthode orElseThrow pour déclencher l'exception
		// GoodException si non trouvé
		accOpt.map(acc -> acc.getOwner()).map(own -> own.getFirstname()).orElseThrow(GoodException::new);
	...
	public void test_getAccountWithPersonFirstnameNotNull() throws Exception {
		...
		accOpt.map(acc -> acc.getOwner())
		      .map(own -> own.getFirstname())
		      .ifPresent(own -> own.equals("A")	);
