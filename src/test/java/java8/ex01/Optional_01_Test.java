...
public class Optional_01_Test {
    ...
    public void test_optional_notfound_throw_exception() throws Exception {
        ...
        List<Person> personList = Data.buildPersonList(100);
       Predicate<Person> pred = p -> { return  p.getAge() == 10  &&  p.getFirstname().equals("last_10"); };

        // TODO invoquer la méthode find(List<T> list, Predicate<T> predicate)
        // TODO age == 10 et firstname == "last_10"
        Optional<Person> result = Optional.ofNullable( find(personList,pred));

        // TODO Utiliser la méthode orElseThrow pour déclencher l'exception NotFountException si non trouvé
        
           
           assertThat( result.orElseThrow(NotFountException::new), hasProperty("age", is(10)));
           assertThat( result.orElseThrow(NotFountException::new), hasProperty("firstname", is("first_10")));
