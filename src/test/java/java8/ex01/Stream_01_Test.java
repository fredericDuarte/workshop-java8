...
public class Stream_01_Test {
	...
	public void test_stream_filter_and_match() throws Exception {
		...
		List<Order> result = orders.stream().filter(x -> x.getCustomer().getFirstname().equals("Johnny"))
				.filter(x -> x.getPizzas().stream().anyMatch(p -> p.getPrice() >= 1300))
	...
	public void test_stream_findFirst() throws Exception {
		...
		Optional<Order> result = orders.stream().filter(x -> x.getCustomer().getFirstname().equals("Sophie")).findAny();
